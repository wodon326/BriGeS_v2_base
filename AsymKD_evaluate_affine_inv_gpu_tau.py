...
@torch.no_grad()
def infer(model_type, model, image, seg_image=None, **kwargs):
    ...
    if seg_image is None:
        if "dpt" in model_type:
            pred1 = model.forward(image, **kwargs).unsqueeze(0)
            pred2 = model.forward(torch.flip(image, [3]), **kwargs).unsqueeze(0)
        else:
...
if "__main__" == __name__:
    ...
    if "depth_anything" in model_type:
        ...
        for child in segment_anything.children():
            ...
            break
        model = AsymKD_DepthAnything_Infer_tau(ImageEncoderViT=ImageEncoderViT).to(device)
        restore_ckpt = "checkpoints_new_loss_001_smooth/116500_AsymKD_new_loss.pth"
    ...
    for data in tqdm(dataloader, desc="Evaluating"):
        ...
        if "depth_anything" in model_type:
            pred = infer(model_type, model, rgb_resized)
        elif model_type == "bfm":
            rgb_resized_seg = segment_anything_predictor.set_image(rgb_float_resized.squeeze())
            pred = infer(model_type, model, rgb_resized, rgb_resized_seg)
            # with torch.no_grad():
            #     pred = model(rgb_resized, rgb_resized_seg)
        elif "dpt" in model_type:
            with torch.no_grad():
                pred = model.forward(rgb_resized).unsqueeze(0)
            # pred = infer(model_type, model, rgb_resized)
