...
def train(rank, world_size, args):
    try:
        ...
        BriGeS.eval()
        ckpt = args.ckpt
        new_state_dict = BriGeS.load_ckpt(ckpt, device=torch.device('cuda', rank))
        if rank == 0:
        ...
        logger = Logger(rank, args.save_dir, keys=["val_student", "val_BriGeS"])
        ...
        while total_steps < args.num_steps:
            for i_batch, data_blob in enumerate(pbar := tqdm(train_loader)):
                ...
                flow_predictions, DA_features, student_trans_features, student_conv_features = model(depth_image)
                assert model.training
                # loss, metrics = sequence_loss(flow_predictions, flow, valid)

                with torch.no_grad():
                    teacher_prediction, teacher_trans_features, teacher_conv_features = BriGeS.forward_with_depth_features(DA_features, depth_image, seg_image)

                feature_loss = [] 

                feat_loss = F.mse_loss(student_conv_features[0], teacher_conv_features[0])
                feature_loss.append(feat_loss)
                
                feat_loss = F.mse_loss(student_conv_features[1], teacher_conv_features[1])
                feature_loss.append(feat_loss)

                feat_loss = F.mse_loss(student_trans_features[2], teacher_trans_features[2])
                feature_loss.append(feat_loss)

                feat_loss = F.mse_loss(student_trans_features[3], teacher_trans_features[3])
                feature_loss.append(feat_loss)

                loss = feature_loss[0] + feature_loss[1] + feature_loss[2] + feature_loss[3]
                ...
                if rank == 0:
                    ...
                    logger.add_scalar("feature_loss/total_loss", loss.item(), total_steps)
                    logger.add_scalar("feature_loss/layer1", feature_loss[0].item(), total_steps)
                    logger.add_scalar("feature_loss/layer2", feature_loss[1].item(), total_steps)
                    logger.add_scalar("feature_loss/layer3", feature_loss[2].item(), total_steps)
                    logger.add_scalar("feature_loss/layer4", feature_loss[3].item(), total_steps)
                    ...
                    for data_blob in vbar:
                        ...
                        with torch.no_grad():
                            if val_flag:
                                ...
                                pred_depths.append(BriGeS(depth_image, seg_image))
                                results = [sequence_loss(pred_depth, flow, valid.bool().unsqueeze(1)) for pred_depth in pred_depths]
                                for model_type, result in zip(["val_student", "val_BriGeS"], results):
                                    loss, metrics = result
                                    logger.push(metrics, model_type)
                            else:
                                pred_depth = model.module(depth_image)
                                model_type = "val_student"
                                loss, metrics = sequence_loss(pred_depth, flow, valid.bool().unsqueeze(1))
                                logger.push(metrics, model_type)

                    for model_type in ["val_student", "val_BriGeS"]:
