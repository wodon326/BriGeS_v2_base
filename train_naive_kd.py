    ...
    try:
        ...
        while total_steps < args.num_steps:
            for i_batch, data_blob in enumerate(pbar := tqdm(train_loader)):
                ...
                flow_predictions, DA_features, student_features = model(depth_image)
                assert model.training
                # loss, metrics = sequence_loss(flow_predictions, flow, valid)

                with torch.no_grad():
                    teacher_prediction, teacher_features = BriGeS.forward_with_depth_features(DA_features, depth_image, seg_image)
