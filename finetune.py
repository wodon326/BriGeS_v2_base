...
def train(rank, world_size, args):
    try:
        ...
        AsymKD_Encoder = AsymKD_Student_Encoder().to(rank)
        AsymKD_Decoder = AsymKD_Student_DPTHead(1,1024).to(rank)

        restore_ckpt = 'checkpoints/AsymKD_KD_Encoder.pth'
        if restore_ckpt is not None:
            assert restore_ckpt.endswith(".pth")
            logging.info("Loading checkpoint...")
            checkpoint = torch.load(restore_ckpt, map_location=torch.device('cuda', rank))
            student_model_encoder_state_dict = AsymKD_Encoder.state_dict()
            new_state_dict = {}
            for k, v in checkpoint.items():
                # 키 매핑 규칙을 정의
                new_key = k.replace('module.', '')  # 'module.'를 제거
                if new_key in student_model_encoder_state_dict:
                    new_state_dict[new_key] = v

            student_model_encoder_state_dict.update(new_state_dict)
            AsymKD_Encoder.load_state_dict(student_model_encoder_state_dict)
        ...
        if restore_ckpt is not None:
            ...
            logging.info("Loading checkpoint...")
            checkpoint = torch.load(restore_ckpt, map_location=torch.device('cuda', rank))
            student_model_decoder_state_dict = AsymKD_Decoder.state_dict()
            new_state_dict = {}
            for k, v in checkpoint.items():
                # 키 매핑 규칙을 정의
                new_key = k.replace('module.depth_head.', '')  # 'module.depth_head.'를 제거
                if new_key in student_model_decoder_state_dict:
                    new_state_dict[new_key] = v

            student_model_decoder_state_dict.update(new_state_dict)
            AsymKD_Decoder.load_state_dict(student_model_decoder_state_dict)
            logging.info(f"Done loading checkpoint")

        AsymKD_student = AsymKD_Student(AsymKD_Encoder,AsymKD_Decoder).to(rank)

    
        model = DDP(AsymKD_student, device_ids=[rank], find_unused_parameters=True)
        #print("Parameter Count: %d" % count_parameters(model))
        print("AsymKD_Student finetune")
        ...
        while should_keep_training:
            ...
            for i_batch, data_blob in enumerate(tqdm(train_loader)):
                optimizer.zero_grad()
                depth_image, _ , flow, valid = [x.cuda() for x in data_blob]
